...
class HandState:
    ...
    Attributes:
        ...
        players_left: Optional number of players remaining in the tournament.
        action_history: Optional description of the action so far from other players.
    """
    ...
    # Optional tournament buy‑in amount
...
def ocr_image(image_path: str) -> Dict[str, str]:
    ...
    try:
        ...
        # Attempt to extract additional tournament context from the top‑left
        # portion of the screenshot (where many poker clients display
        # buy‑in and number of players remaining).  We crop roughly
        # the upper 20% and left 30% of the frame and run OCR on
        # that region separately.
        players_left = ""
        buy_in = ""
        try:
            h, w = img.shape[:2]
            crop = img[0 : int(h * 0.25), 0 : int(w * 0.35)]
            scoreboard_gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
            _, scoreboard_thresh = cv2.threshold(
                scoreboard_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
            )
            scoreboard_text = pytesseract.image_to_string(scoreboard_thresh)
            import re

            # Look for patterns like '35 / 62' to infer players left (62)
            match = re.search(r"(\d+)\s*/\s*(\d+)", scoreboard_text)
            if match:
                # second number is total players remaining/registered
                players_left = match.group(2)
            # Look for patterns like 'Buy-in $2' or 'Buy in $2'
            match2 = re.search(
                r"Buy[-\s]?in\s*\$?([\d\.]+)", scoreboard_text, re.IGNORECASE
            )
            if match2:
                buy_in = match2.group(1)
        except Exception:
        ...
        result = {
            ...
            "board": " ".join(board) or "",
            "pot": pot or "",
            # Additional fields for tournaments. The OCR implementation
            # does not currently extract these, so they are left blank
            # for manual entry later in the UI.
            "players_left": players_left or "",
            "action_history": "",
            "buy_in": buy_in or "",
            "raw_text": raw_text,
        }
        ...
        return {
            ...
            "board": "7c 8d 2s",
            "pot": "5500",
            # Placeholder values for tournament context
            "players_left": "6",
            "action_history": "HJ opens 2.2bb",
            "buy_in": "2",
        }
...
def parse_hand_state(ocr_output: Dict[str, str]) -> HandState:
    ...
    # Convert players_left to integer if available
    players_left_str = ocr_output.get("players_left")
    ...
    return HandState(
        ...
        players_left=to_int(players_left_str),
        action_history=ocr_output.get("action_history"),
        buy_in=to_float(ocr_output.get("buy_in")),
    )
...
def general_concept_analysis(state: HandState) -> str:
    ...
    # Add ICM considerations based on players remaining
    icm_note = ""
    if state.players_left is not None and state.players_left > 0:
        # Rough heuristic: final table or bubble when <= 6 players, moderate ICM for <= 18
    ...
    return f"{base} {position_note} {hand_note} {icm_note}"
